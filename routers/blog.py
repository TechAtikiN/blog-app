...
def create_a_blog( request: schemas.Blog, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
  return blog.create(request, db)

# delete a blog from database
@router.delete('/{id}', status_code=status.HTTP_204_NO_CONTENT)
def delete_a_blog(id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
  return blog.delete(id, db)

# update a blog in database
@router.put('/{id}', status_code = status.HTTP_202_ACCEPTED)
def update_a_blog(id: int, request: schemas.Blog, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
